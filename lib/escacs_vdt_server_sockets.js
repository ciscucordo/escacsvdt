...
//exports.addSocketIOEvents = function (pServerHTTP) {
    ...
    io.sockets.on("connection", function (pSocket) {
        ...
        //quan un jugador fa escac i mat
        handleDoCheckMate(pSocket);
...
function handleDoCheckMate(pSocket) {
    pSocket.on("checkmate", function (pDoCheckMate) {
        pSocket.broadcast.to(pDoCheckMate.room).emit("checkmate", {
            nickGuanyador: pDoCheckMate.nickGuanyador,
            colorGuanyador: pDoCheckMate.colorGuanyador
        });
    });
}