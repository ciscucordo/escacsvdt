...
//exports.addSocketIOEvents = function (pServerHTTP) {
    ...
    io.sockets.on("connection", function (pSocket) {
        ...
        //permet enviar a una notificació informant que ja pot començar la partida
        handleCanBeginGame(pSocket);
        //quan un jugador perd (resign, time, disconnect ó checkmate)
        handleFinishGame(pSocket);
        ...
        handleMessageBroadcasting(pSocket);
...
function joinRoom(pSocket, pRoom) {
    ...
    pSocket.broadcast.to(pRoom.newRoom).emit("systemMessageBroadcastJoinRoom", {
        text: nick.trim(),
        textAlreadyInRoom: nicksAlreadyInRoom.trim()
    });
...
function handleMessageBroadcasting(pSocket) {
    pSocket.on("messagy", function (pMessage) {
        pSocket.broadcast.to(pMessage.room).emit("messagy", {
...
function handleCanBeginGame(pSocket) {
    pSocket.on("canBeginGame", function () {
        var room = mainRoom[pSocket.id];
        pSocket.broadcast.to(room).emit("canBeginGame", {
            beginGame: true
        });
    });
}

function handleFinishGame(pSocket) {
    pSocket.on("finishGame", function (pFinishGame) {
        var room = mainRoom[pSocket.id];
        pSocket.broadcast.to(room).emit("finishGame", {
            nickLoser: pFinishGame.nickLoser,
            colorLoser: pFinishGame.colorLoser,
            //'resign', 'time', 'disconnect' ó 'checkmate'
            tipusFinish: pFinishGame.tipusFinish
        });
    });
}
