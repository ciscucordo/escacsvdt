...
exports.doMySqlOp = function (pProcess, pRequest, pResponse) {
    if (!db) {
        db = mysql.createConnection({
            host: pProcess.env.OPENSHIFT_MYSQL_DB_HOST || '127.0.0.1',
            user: pProcess.env.OPENSHIFT_MYSQL_DB_USERNAME || 'escacsvdt',
            password: pProcess.env.OPENSHIFT_MYSQL_DB_PASSWORD || 'escacsvdt',
            port: pProcess.env.OPENSHIFT_MYSQL_DB_PORT || 3306,
            database: pProcess.env.OPENSHIFT_GEAR_NAME || 'escacsvdt'
        });
    }
    var isMySqlOp = false;
    ...
    switch (op) {
        ...
        case "dbProperties":
            isMySqlOp = true;
            utils.sendHtml(pResponse, pProcess.env.OPENSHIFT_MYSQL_DB_HOST +
                    "|" + pProcess.env.OPENSHIFT_MYSQL_DB_USERNAME + 
                    "|" + pProcess.env.OPENSHIFT_MYSQL_DB_PASSWORD + 
                    "|" + pProcess.env.OPENSHIFT_MYSQL_DB_PORT +
                    "|" + pProcess.env.OPENSHIFT_GEAR_NAME);
            break;
    ...
    utils.parseReceivedData(pRequest, function (pRow) {
        ...
        db.query(
                ...
                function (err, rows) {
                    ...
                    utils.sendHtml(pResponse, rows.length);
                    /*if (rows.length === 1) {
